...
| Domain | 설명 |
|:--------|:-----|
| **Accounts** | 회원가입, 로그인(JWT), 사용자 정보 및 주소 관리 |
| **Catalog** | 카테고리 / 상품 / 재고 관리 (`option_key` 기반 옵션) |
| **Carts** | 장바구니 CRUD, 옵션별 수량 및 재고 검증 |
| **Orders** | Checkout, 주문 생성 및 상태 관리 (paid / canceled / refunded) |
| **Payments** | Toss Payments 테스트 API 연동 (승인 / 취소 / 환불) |
| **Shipments** | 배송지 및 출고 처리 로직 |
| **Admin API** | 관리자용 상품/주문/사용자 관리 (권한 기반 접근 제어) |
