...
from domains.payments.models import Payment as PaymentModel, PaymentEvent as PaymentEventModel
...
class TestConfirmPayment:
    ...
    def test_confirm_payment_success(self, user_factory, product_factory):
        ...
        # 결제 승인
        with patch('domains.carts.services.get_user_cart') as mock_get_cart, \
        ...
        # 이벤트 기록 확인
        events = PaymentEventModel.objects.filter(payment=payment, event_type="approval")
        assert events.count() == 1
        event = events.first()
