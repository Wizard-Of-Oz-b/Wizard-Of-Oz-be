...
from domains.catalog.models import Product, Category
...
class TestAddOrUpdateItem:
    ...
    def test_invalid_quantity(self, user_factory, product_factory):
        ...
        with pytest.raises(ValueError, match="quantity must be >= 1"):
            add_or_update_item(
                user=user, product=product, options={}, quantity=0
            )
        
        with pytest.raises(ValueError, match="quantity must be >= 1"):
            add_or_update_item(
                user=user, product=product, options={}, quantity=-1
            )
    ...
    def test_invalid_options_type(self, user_factory, product_factory):
        ...
        with pytest.raises(ValueError, match="options must be dict or"):
            add_or_update_item(
                user=user, product=product, options=123, quantity=1
            )
...
class TestClearCart:
    ...
    def test_clear_cart_with_items(self, user_factory, product_factory):
        ...
        CartItem.objects.create(
            cart=cart, product=product, quantity=1, unit_price=Decimal(str(product.price))
        )
