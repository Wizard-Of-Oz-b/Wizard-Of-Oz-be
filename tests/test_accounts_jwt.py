...
from domains.accounts.jwt import (
...
class TestEmailTokenObtainPairSerializer:
    ...
    def test_validate_user_not_found(self):
        """존재하지 않는 사용자 테스트"""
        serializer = EmailTokenObtainPairSerializer()
        attrs = {"email": "nonexistent@example.com", "password": "testpass123"}

        with pytest.raises(AuthenticationFailed) as exc_info:
            serializer.validate(attrs)

        assert "지정된 자격 증명에 해당하는 활성화된 사용자를 찾을 수 없습니다" in str(
            exc_info.value
        )
        assert (
    ...
    def test_validate_wrong_password(self, user_factory):
        ...
        serializer = EmailTokenObtainPairSerializer()
        attrs = {"email": "test@example.com", "password": "wrongpass123"}
    ...
    def test_validate_none_email(self):
        """None 이메일 테스트"""
        serializer = EmailTokenObtainPairSerializer()
        attrs = {"email": None, "password": "testpass123"}
    ...
    def test_validate_none_password(self, user_factory):
        ...
        serializer = EmailTokenObtainPairSerializer()
        attrs = {"email": "test@example.com", "password": None}
...
class TestJWTIntegration:
    ...
    def test_complete_jwt_flow(self, user_factory):
        ...
        # 사용자 생성
        user = user_factory(
            email="integration@example.com", password="integrationpass123"
        )

        # 시리얼라이저로 토큰 발급
        serializer = EmailTokenObtainPairSerializer()
        attrs = {"email": "integration@example.com", "password": "integrationpass123"}
    ...
    def test_multiple_users_different_emails(self, user_factory):
        ...
        # 첫 번째 사용자
        attrs1 = {"email": "user1@example.com", "password": "pass1"}
        result1 = serializer.validate(attrs1)

        # 두 번째 사용자
        attrs2 = {"email": "user2@example.com", "password": "pass2"}
    ...
    def test_same_user_different_case_emails(self, user_factory):
        ...
        for email in test_emails:
            attrs = {"email": email, "password": "testpass123"}
            result = serializer.validate(attrs)
