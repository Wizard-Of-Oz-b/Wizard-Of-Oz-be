...
class TestCheckoutUserCart:
    ...
    def test_checkout_user_cart_success(self, user_factory, product_factory):
        ...
        ProductStock.objects.create(
            product=product, option_key="size=L", stock_quantity=10
        )
    ...
    def test_checkout_user_cart_insufficient_stock(self, user_factory, product_factory):
        ...
        # 재고 생성 (부족한 수량)
        ProductStock.objects.create(
            product=product, option_key="size=L", stock_quantity=1
        )
...
class TestCheckout:
    ...
    def test_checkout_success(self, user_factory, product_factory):
        ...
        # 체크아웃 실행
        with patch(
            "domains.payments.services.create_payment_stub"
...
class TestRefundPurchase:
    ...
    def test_refund_purchase_success(self, user_factory, product_factory):
        ...
        # 재고 생성
        ProductStock.objects.create(
            product=product, option_key="size=M", stock_quantity=2
        )
...
class TestCreateOrderItemsFromCart:
    ...
    def test_create_order_items_from_cart_success(self, user_factory, product_factory):
        ...
        # 구매 생성
        purchase = Purchase.objects.create(user=user, status=Purchase.STATUS_READY)
    ...
    def test_create_order_items_from_cart_idempotent(
        ...
        OrderItem.objects.create(
            order=purchase, product=product, quantity=1, unit_price=Decimal("10000")
        )
    ...
    def test_create_order_items_from_cart_insufficient_stock(
        self, user_factory, product_factory
    ):
        """재고 부족 시 테스트"""
        user = user_factory()
