...
class TestSyncByTracking:
    ...
    def test_sync_by_tracking_success(self, user_factory, product_factory):
        ...
        # 동기화 실행
        with patch('domains.shipments.services.upsert_events_from_adapter') as mock_upsert:
    ...
    def test_sync_by_tracking_default_adapter(self, user_factory, product_factory):
        ...
        with patch('domains.shipments.adapters.get_adapter') as mock_get_adapter, \
             patch('domains.shipments.services.SweetTrackerAdapter') as mock_sweet_tracker, \
             patch('domains.shipments.services.upsert_events_from_adapter') as mock_upsert:
...
class TestUpsertEventsFromAdapter:
    ...
    def test_upsert_events_success(self, user_factory, product_factory):
        ...
        # 이벤트 업서트
        with patch('domains.shipments.services._recompute_status_from_events') as mock_recompute, \
...
class TestShipmentsServicesEdgeCases:
    ...
    def test_upsert_events_invalid_datetime_skip(self, user_factory, product_factory):
        ...
        product = product_factory()
        order = Purchase.objects.create(
            user=user, product=product, amount=1, unit_price=Decimal("10000"), status="paid"
        )
        shipment = Shipment.objects.create(
            carrier="kr.cjlogistics", tracking_number="CJT-123456", user=user, order=order
        )
    ...
    def test_upsert_events_notification_exception_handling(self, user_factory, product_factory):
