...
class TestReserveStock:
    ...
    def test_reserve_stock_insufficient(self, product_factory):
        ...
        ProductStock.objects.create(
            product=product, option_key="size=L", stock_quantity=3
        )
    ...
    def test_reserve_stock_create_missing_row(self, product_factory):
        ...
        # (예외 발생 시 트랜잭션이 롤백되므로)
        assert not ProductStock.objects.filter(
            product=product, option_key="size=M"
        ).exists()
    ...
    def test_reserve_stock_zero_quantity(self, product_factory):
        ...
        # 재고 행이 생성되지 않았는지 확인
        assert not ProductStock.objects.filter(
            product=product, option_key="size=L"
        ).exists()
...
class TestGetStockQuantity:
    ...
    def test_get_stock_quantity_existing(self, product_factory):
        ...
        # 재고 생성
        ProductStock.objects.create(
            product=product, option_key="size=L", stock_quantity=15
        )
...
class TestStockIntegration:
    ...
    def test_multiple_option_keys(self, product_factory):
        ...
        # 여러 옵션의 재고 생성
        ProductStock.objects.create(
            product=product, option_key="size=S", stock_quantity=5
        )
        ProductStock.objects.create(
            product=product, option_key="size=M", stock_quantity=10
        )
        ProductStock.objects.create(
            product=product, option_key="size=L", stock_quantity=15
        )
