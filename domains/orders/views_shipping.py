...
class UpdatePurchaseAddressAPI(views.APIView):
    ...
    def patch(self, request, purchase_id):
        ...
        if order.status not in (Purchase.STATUS_READY, PurchaseStatus.READY):
            return Response(
                {"detail": "address change not allowed after payment"}, status=409
            )

        payload = request.data or {}
        if payload.get("address_id"):
            a = get_object_or_404(
                UserAddress, pk=payload["address_id"], user=request.user, is_active=True
            )
            order.shipping_recipient = a.recipient
            order.shipping_phone = a.phone
            order.shipping_postcode = a.postcode
            order.shipping_address1 = a.address1
            order.shipping_address2 = a.address2
            order.shipping_memo = payload.get("memo", "")
        elif payload.get("address"):
            addr = payload["address"]
            order.shipping_recipient = addr.get("recipient", "")
            order.shipping_phone = addr.get("phone", "")
            order.shipping_postcode = addr.get("postcode", "")
            order.shipping_address1 = addr.get("address1", "")
            order.shipping_address2 = addr.get("address2", "")
            order.shipping_memo = addr.get("memo", "")
        else:
...
@extend_schema(
    summary="현재 사용자의 모든 ready 주문 배송지 일괄 변경", tags=["Orders"]
)
        ...
        request={
            "application/json": {
                ...
                "properties": {
                    ...
                    "memo": {"type": "string", "description": "배송 메모 (선택사항)"},
                },
            }
        },
        responses={
    ...
    def patch(self, request):
        ...
        if address_id:
            ...
            try:
                ...
                return Response(
                    {"detail": "UserAddress not found or not active"}, status=404
                )
        elif inline_address:
            # 인라인 주소 정보 사용
            address_data = {
        ...
        orders_to_update = Purchase.objects.filter(
            user=user, status__in=[Purchase.STATUS_READY, PurchaseStatus.READY]
        ).select_for_update()
        ...
        # 업데이트된 주문 ID 목록
        updated_purchase_ids = list(
            orders_to_update.values_list("purchase_id", flat=True)
        )
