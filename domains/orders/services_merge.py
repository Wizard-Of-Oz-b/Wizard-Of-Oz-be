...
def get_user_ready_orders_summary(user: Any) -> dict:
    ...
    ready_orders = Purchase.objects.filter(
        user=user,
        status=Purchase.STATUS_READY
    ).prefetch_related('items').order_by('-purchased_at')
...
def cancel_merged_order(merged_order: Purchase, user: Any) -> dict:
    ...
    original_orders.exclude(purchase_id=first_original_order.purchase_id).update(status=Purchase.STATUS_READY)
