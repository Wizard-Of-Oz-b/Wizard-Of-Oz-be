...
class MyAddressListCreateAPI(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        return (UserAddress.objects
                .filter(user=self.request.user, is_active=True)
                .order_by("-is_default","-created_at"))

    def get_serializer_class(self):
        return UserAddressWriteSerializer if self.request.method == "POST" else UserAddressReadSerializer
...
class MyAddressDetailAPI(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_serializer_class(self):
        return UserAddressWriteSerializer if self.request.method in ("PATCH","PUT") else UserAddressReadSerializer
...
class SetDefaultAddressAPI(APIView):
    ...
    def post(self, request, address_id):
        ...
        UserAddress.objects.filter(user=u, is_default=True).exclude(pk=addr.pk).update(is_default=False)
