...
class Payment(models.Model):
    ...
    # 주문 헤더 모델명이 Purchase 라서 이 참조가 맞습니다
    order = models.ForeignKey("orders.Purchase", on_delete=models.CASCADE, related_name="payments")

    provider = models.CharField(max_length=20, choices=PaymentProvider.choices, default=PaymentProvider.TOSS)
    provider_payment_key = models.CharField(max_length=200, unique=True, null=True, blank=True)  # Toss paymentKey
    order_number = models.CharField(max_length=100, unique=True, null=True, blank=True)          # Toss orderId(=내부 주문번호)

    method = models.CharField(max_length=30, choices=PaymentMethod.choices, null=True, blank=True)
    status = models.CharField(max_length=32, choices=PaymentStatus.choices, default=PaymentStatus.READY)
...
class PaymentEvent(models.Model):
    ...
    provider_status = models.CharField(max_length=32, choices=PaymentStatus.choices, null=True, blank=True)
...
class PaymentCancel(models.Model):
    ...
    # ✅ order_items 도입 후에 다시 연결하세요
    # order_item = models.ForeignKey("orders.OrderItem", on_delete=models.SET_NULL, null=True, blank=True, related_name="payment_cancels")

    status = models.CharField(max_length=16, choices=CancelStatus.choices, default=CancelStatus.REQUESTED)
